...
# Crear una IU de autenticación {#create-an-authentication-ui}
...
1. Escriba el método `credentials()` para procesar los valores de autenticación proporcionados por el usuario.

   El método `credentials()` es un método definido por el usuario que pasa los valores de nombre de usuario y contraseña al método `setDRMAuthenticationCredentials()`. Una vez pasados los valores, el método `credentials()` restablece los valores de los objetos `TextInput`.
