...
* Tablero SSAI: Estadísticas de sesión. Puede exportar los eventos de sesión mediante la opción Descargar CSV.
* Actualizaciones de mantenimiento
...
* Tablero SSAI: Se ha agregado una nueva sección, Estadísticas de sesión, al panel de SSAI
   * Si tiene habilitado el identificador de sesión para una sesión SSAI con el modo de depuración (ptdebug=true), podrá buscar la siguiente actividad que se produjo en esa sesión:
...
* CRS: Regla de normalización para elementos creativos de RevJet
   * Se agregó la regla de normalización de URL creativa para RevJet, utilizada por CRS y SSAI
...
* CRS: Regla de normalización para los creativos de Innovid
   * Se agregó la regla de normalización de URL creativa para Innovid, utilizada por SSAI
...
* Se agregó compatibilidad con CMAF (contenido HLS/fMP4)
...
* SSAI: Se agregó compatibilidad con tokens de autenticación CDN de Limelight (LLNW) para segmentos de contenido
   * Cuando `pttoken=limelight` o `pttoken=llnw` se agregue a la URL de arranque, agregaremos un encabezado secreto al recuperar la lista de reproducción maestra de origen y, a continuación, adjuntaremos los parámetros de consulta del encabezado X-Adobe-Sig de LLNW a los segmentos de contenido
* SSAI: Se ha agregado otro valor pttoken (`pttoken=centurylink`) para la compatibilidad con autentificadores CDN de CenturyLink, que se publicó el 30 de julio de 2018
...
* Tablero CRS: Se ha truncado la cadena &quot;Ejemplo de FqAdId&quot; a 255 caracteres debido a restricciones de almacenamiento de datos (8 bits)
...
* Tableros SSAI y CRS: Actualizaciones de versiones de software
...
* CRS: Se ha rediseñado la arquitectura y el flujo de trabajo para cargar recursos de CRS a los orígenes de CDN de los clientes
   * Los procesos de trabajo por origen CDN se separan, por lo que los cuellos de botella de carga para un origen CDN no afectarán a las cargas a otros orígenes CDN
...
* SSAI: Se agregaron las direcciones URL de ClickThrough y ClickTracking para anuncios de vídeo al formato JSON v2 sidecar
   * Una nueva propiedad de matriz JSON, &quot;videoClicks&quot;, seguirá la propiedad &quot;trackingURLs&quot;
   * Los nombres de los valores &quot;event&quot; serán &quot;clickThrough&quot; y &quot;clickTracking&quot;, y no tendrán un valor startTime
...
* SSAI: Se ha actualizado el comportamiento de manipulación de manifiestos de WebVTT solo para transmisiones en directo
   ...
   * Nuevo comportamiento: Se ha agregado un nuevo parámetro, vttdisk, con valores aceptados de true y false, a la URL de inicio de SSAI
...
* SSAI: Se agregaron las direcciones URL de ClickThrough y ClickTracking para anuncios de vídeo al formato JSON v2 sidecar
   * Bajo la propiedad &quot;trackingURLs&quot;, los nombres de los valores &quot;event&quot; serán &quot;clickthrough&quot; y &quot;clickTracking&quot;
...
* SSAI: Actualizaciones de versiones de software para NGINX y Kafka
   * NGINX y Kafka se utilizan para activar direcciones URL de seguimiento de anuncios en el servidor y para enviar datos a los tableros SSAI y CRS
...
* Se ha corregido el problema de la interfaz de usuario del calendario por el que el usuario no podía seleccionar una fecha posterior al 31 de diciembre de 2018 en el componente de calendario al enviar una campaña o extraer un informe.
...
* SSAI: Se ha actualizado la estructura de clave de búsqueda que utiliza SSAI para almacenar y recuperar recursos de CRS, con el fin de controlar escenarios en los que los proveedores de publicidad tienen un ID de publicidad dinámico o un ID creativo para la misma publicidad
   * Nueva estructura de clave de búsqueda: Parámetros de zona, URL creativa y formato (duración de destino, formato de salida, CDN de destino)
   * Estructura de clave de búsqueda antigua: Zona, sistema de publicidad, ID de publicidad, ID creativo, URL creativa y parámetros de formato (duración del objetivo, formato de salida, CDN de destino)
...
* CRS: Se ha agregado la capacidad de incluir en una lista negra o lista blanca solicitudes CRS de sistemas de publicidad específicos, ID de publicidad, ID creativos, URL creativas y/o formato creativo
...
* Se corrigió un problema en el cual las comillas simples (&#39;) y las comillas dobles (&quot;) en ID de publicidad, ID creativo y otros campos para una solicitud de reempaquetado ocasionaban que la solicitud de reempaquetado fallara.
...
ZD#37503 - Las respuestas de Json para las reglas de CRS se almacenan en caché para evitar las solicitudes duplicadas.
